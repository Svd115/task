		...
		if(/^\s*$/.test(email)){
			...
			if(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)){
				...
				forget.on('return', function(data){
					if(data[0]){
						var content = 
						"<p>In order to reset your password please click on this link : </p><a href='"+req.protocol+"://"+req.hostname+"/forget/"+data[1]+"/"+data[2]+"'>Reset my password</a><p>If the link doesn't work, please copy this : "+req.protocol+"://"+req.hostname+"/signup/"+data[1]+"/"+data[2]+"</p>";
